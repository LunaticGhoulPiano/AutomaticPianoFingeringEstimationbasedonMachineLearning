...
class Parser(mido.MidiFile):
    def __init__(self, filename, path):
        try:
            if not filename.endswith('.mid'):
                filename += '.mid'
            self.midifile = mido.MidiFile(f'./MidiData./MidiFiles./{filename}')
        except:
    ...
    def writeMidiToTxt(self):
        with open(f'{self.path}./{self.filename}.txt', 'w') as f:
    ...
    def getEvents(self, generateMergedFingerings):
        ...
        time_signatures = []
        tempos = []
        # to maintain the multipart relationship
        ori_order = 0
        for i, track in enumerate(self.midifile.tracks):
        ...
        if len(tracks) == 2:
            ...
            right_notes = sorted(right_notes, key = lambda note: note.original_order)
            tracks[0] = right_notes
            left_notes = sorted(tracks[1], key = lambda note: (note.onset_time_in_sec, note.pitch))
            left_notes = sorted(left_notes, key = lambda note: note.original_order)
            tracks[1] = left_notes

            # write note messages with precise time # just for debug
            with open(f'{self.path}./track_records.txt', 'w') as f:
            ...
            with open(f'{self.path}./left_features.csv', 'w', newline = '') as left, \
                open(f'{self.path}./right_features.csv', 'w', newline = '') as right, \
                open(f'{self.path}./merged_features.csv', 'w', newline = '') as merged:
                # write headers
                ...
                # merged two hands
                merged_notes = left_notes + right_notes
                
                if generateMergedFingerings:
                    # read true fingerings
                    left_true_Begin_df = pd.read_csv(f"./MidiData./TrueFingerings./{self.filename[:-4]}./left_true.csv")
                    right_true_Begin_df = pd.read_csv(f"./MidiData./TrueFingerings./{self.filename[:-4]}./right_true.csv")
                    # merge two true fingerings
                    merged_true_Begin_df = pd.concat([left_true_Begin_df, right_true_Begin_df], ignore_index = True)
                    # sort fingerings by 'Note_number'
                    temp_df = pd.DataFrame({'Note_number': [note.note_number for note in merged_notes], 'Begin_fingering': merged_true_Begin_df['Begin_fingering'], 'End_fingering': merged_true_Begin_df['End_fingering']})
                    sorted_merged_true_Begin_df = temp_df.sort_values(by = 'Note_number').reset_index(drop = True)
                    sorted_merged_true_Begin_df = sorted_merged_true_Begin_df.drop(['Note_number'], axis = 1)
                    # write merged fingering file
                    sorted_merged_true_Begin_df.to_csv(f'./MidiData./TrueFingerings./{self.filename[:-4]}./merged_true.csv', index = False)
                
                # sort notes by 'Note_number'
                merged_notes = sorted(merged_notes, key = lambda note: note.note_number)
                # write merged notes to merged file
...
def main(filename, path, generateMergedFingerings = False):
    parser = Parser(filename, path)
    parser.judgeInsrtumentType()
    parser.writeMidiToTxt() # just for debug
    parser.getEvents(generateMergedFingerings)